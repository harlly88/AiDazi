...
Future<bool> writeFile(String data) async {
  try {
    ...
    final anchor = html.AnchorElement(href: url)
      ..setAttribute('download', 'aiDaziBackup_${getTimeStr(DateTime.now().millisecondsSinceEpoch)}.json')
      ..click();
...
Future<bool> writePngFile(Uint8List data) async {
  try {
    ...
    final url = html.Url.createObjectUrlFromBlob(blob);
    final anchor = html.AnchorElement(href: url)
      ..setAttribute('download', 'aiDaziCard_${getTimeStr(DateTime.now().millisecondsSinceEpoch)}.png')
      ..click();
    html.Url.revokeObjectUrl(url);
    return true;
  } catch (e) {
